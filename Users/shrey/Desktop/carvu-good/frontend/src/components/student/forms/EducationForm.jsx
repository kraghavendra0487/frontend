        ...
        <VStack spacing={6} align="stretch" mt={4}>
          <SimpleGrid columns={{ base: 1, md: 2 }} gap={6}>
            ...
            <FormControl isInvalid={!!getError('year_of_passing')}>
              <Field label="Year of Passing *">
                <Input 
                  ...
                  onChange={(e) => {
                    ...
                    // Only allow 4-digit years and not greater than current year
                    if (val === '' || (val.length <= 4 && /^\d+$/.test(val))) {
                      const numVal = parseInt(val, 10);
                      if (val.length === 4 && numVal > currentYear) {
                        // Don't update if year is in the future
                        return;
                      }
              ...
              {getError('year_of_passing') && (
                <Text fontSize="sm" color="red.500" mt={1}>{getError('year_of_passing')}</Text>
              )}
            </FormControl>
          ...
          <Box bg="gray.50" p={4} borderRadius="md">
            ...
            <SimpleGrid columns={{ base: 1, md: 3 }} gap={6}>
                 <Field label="Gap Type" helperText="e.g. 12th to Graduation">
                    <Select variant="flushed" isDisabled={!isEditing} _disabled={{ opacity: 1, cursor: "default", bg: "gray.100", px: 2, py: 1, borderRadius: "md", color: "gray.800" }} value={item.gapType ?? ""} onChange={(e) => handleChange("gapType", e.target.value)} placeholder="Select">
                ...
                <FormControl isInvalid={!!getError('gap_duration_months')}>
                  <Field label="Duration (Months)" helperText="Use 00 if no gap (00-99)">
                      <Input 
                        type="text" 
                        inputMode="numeric"
                        ...
                        onBlur={(e) => {
                          ...
                          // Auto-pad with leading zero if single digit (e.g., 5 -> 05)
                          if (val !== '' && /^\d{1}$/.test(val)) {
                            handleChange("gapDurationMonths", val.padStart(2, '0'));
                          }
                        }}
                        variant="flushed" 
                        isDisabled={!isEditing} 
                  ...
                  {getError('gap_duration_months') && (
                    <Text fontSize="sm" color="red.500" mt={1}>{getError('gap_duration_months')}</Text>
                  )}
                </FormControl>
                <Field label="Reason" helperText="e.g. N/A or reason">
                    <Input value={item.gapReason ?? ""} onChange={(e) => handleChange("gapReason", e.target.value)} variant="flushed" isDisabled={!isEditing} _disabled={{ opacity: 1, cursor: "default", bg: "gray.100", px: 2, py: 1, borderRadius: "md", color: "gray.800" }} placeholder="N/A if no gap" />
                </Field>
            </SimpleGrid>
          </Box>
          </VStack>
